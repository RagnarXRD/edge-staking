...
export default function Home() {
	...
	const { accountAddress, edgeBalance, sEdgeBalance } = useAppSelector(
		(state) => state.accountAddressReducer,
	);
	const { sEdg_totalSupply } = useAppSelector(
		(state) => state.setSedgSupplyReducer,
	);
	...
	const apy = useMemo(
		() => ((yearlyEdgReward / +sEdg_totalSupply) * 100).toFixed(2),
		[sEdg_totalSupply],
	);
			...
			<div className="flex w-full h-20 items-center justify-center">
				<div className="flex w-full justify-between items-center h-full">
					<div className="flex items-center gap-2">
						<div
							className={`p-1 bg-red-200 rounded-full shadow-md ${styles.colorChange200}`}
						>
							<div
								className={`p-1 bg-red-300 rounded-full shadow-md ${styles.colorChange300}`}
							>
								<div className="flex items-center">
									<div
										className={`p-1 bg-red-400 rounded-full ${styles.colorChange400}`}
									>
										<Image
											src="/new logo800.png"
											width={60}
						...
						<h1
							className={`hidden md:block font-bold text ${styles.colorChange}`}
						>
							EDGE
						</h1>
					</div>
